  ...
  <div class="col-5 px-4">
    <div class="bg-white">
      ...
      <div class="messages-box">
        <div class="list-group rounded-0"> @foreach($message->sortBy('created_at') as $value) 
          @if(Auth::user()->id != $value->sender_id) 
          <a href="{{url('manager/message/view')}}/{{$value->sender_id}}" class="list-group-item list-group-item-action list-group-item-light rounded-0">
                @elseif (Auth::user()->id != $value->user_id)
                <a href="{{url('manager/message/view')}}/{{$value->user_id}}" class="list-group-item list-group-item-action list-group-item-light rounded-0">
                @else
              ...
              <div class="media">
                ...
                <div class="media-body ml-4">
                  ...
                  <div class="row">
                  <p class="font-italic text-muted mb-0 text-small"style="padding-left:12px;" >{{$value->subject}}</p>
                  <h6 style="padding-left:120px;"><span class="badge badge-danger badge-counter">{{DB::table('messages')->where('msg_status','=',0)->where('user_id',Auth::user()->id)->count()}}</span></h6>
                  </div>
                </div>
              </div>
            </a> @endforeach </div>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form class="form-horizontal" role="form" method="post" action="{{url('manager/message/save')}}" enctype="multipart/form-data"> {{ csrf_field() }}
          <div class="form-group">
            <select class="form-control dynamic" name="user_type_id" id="user_type_id" type="select" data-dependent="emp_no" required>
              <option>Job Role</option> 
              @foreach(App\user_type::all() as $data)
